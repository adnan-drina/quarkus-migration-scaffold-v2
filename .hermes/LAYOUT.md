...
| Planning contracts | `.hermes/planning/` (JSON schemas for the evidence bundle, the work list, the admission receipt and `decisions.yaml`; catalogs incl. `compat-mapping.json`; MTA rules incl. `rhoai3-canary`; `decisions.example.yaml`). Planner code is `.hermes/lib/planner/` (`worklist`, `cards`, `admission`, `roadmap`, `evidence`, `live_board`, `pins`, `decisions`). The only human input is `decisions.yaml` at the project root (platform, attempt threshold, ADR-retired items). Run budget: `run-defaults.json` (golden: shared limits and pins, never readiness) is bound by `run-budget.json`, which the app-migration factory writes into the destination's initial commit with the run's name; `planner/run_declaration.py` refuses a missing, foreign, stale or rewritten declaration and gates autostart. |
| Planning skills | `.hermes/skills/planning/` (`build-worklist`, `admit-migration-plan`, `compose-serial-roadmap` derived serial view, `verify-live-kanban-loop`) |
...
| Shared Python | `.hermes/lib/` — **not a skill** (`planner/` package: canonical digests, yamlite, schema_lite, evidence, worklist, cards, admission, roadmap, live_board, pins, decisions; plus generated_sources, specimen splits, paved_road, check-external-dirs). Identity is `.hermes-lib` marker, not a member module. |
| Paved-road | `.hermes/skills/paved-road/` — kind index (`paved-road-m1` … `paved-road-m4`; `paved-road-m5`: eligibility-checked start → prepare candidate → observe app-push (TaskRun `IMAGE_DIGEST`) → assert ready app (Deployment or fenced pod view) / HTTPS Route → live Route acceptance → compose M5 verdict). `steps.json` generates `audit.json`. M5 is an assisted continuation after M4 close with a separate budget; the M4 worker never dest-dispatches it. M4 `ship: false` is historical; M5 `ACCEPT` is the existing release contract plus pinned kill-ratio evidence. Application values live in `delivery.yaml`. Hermes 0.20.5 mint omits `--initial-status todo`. |
...
| Outcome board (new runs only, **disabled by default**) | Contract: `stages/080-ai-autonomous-migration/OUTCOME-BOARD-CONTRACT.md`. Selected by `run-defaults.json` `configuration.board_protocol: outcome-board/v1` (bound by the initial commit) or the platform run-control contract; absent = the serial loop above, unchanged. Execution: `pins.planner.outcome_board.execution` ∈ disabled (default) / qualification (disposable fixture only) / enabled (refuses `AUTHORITY_UNPROTECTED` until a protected writer exists). Planner: `lib/planner/outcome_{protocol,graph,store,lifecycle,native,hook}.py`. K4: `kernel/k4_graph.py` (reached from `k4_mint.py`; publishes the whole graph under the open M2, identity retained). Worker entry points: `kernel/outcome_gate.py` (issue, verdict, accept-commit, restore-pending, assessment-record, stage-result, effect-admit/record, account). Continuations: `kernel/outcome_reconcile.py` on the dispatcher's `on_kanban_dispatch_tick`. Authority store: `verification/outcome-board/authority.sqlite3` (cooperative; K2 refuses tool writes to it). Mixed serial/outcome records refuse everywhere. `claimed_control` stays false. |
| Run data | `evidence/` |
...
| `paved-road-m5` | paved-road | M5 delivery index; pin on M5 PREFLIGHT / DEPLOY / VALIDATE; eligibility-checked start after M4 close |
...
| `compose-serial-roadmap` | planning | Derived serial view after M2: one executable next card; planned M4 VERIFY and M5 PREFLIGHT / DEPLOY / VALIDATE without candidate/receipt claims. Not sealed; not a KEEP/audit step |
