...
| rest-cors-compile | `cannot find symbol: class CrossOrigin` (the Spring Web compatibility API does not ship it) | Delete the `@CrossOrigin` annotation (class-level or method-level, with its arguments) and its import; nothing else changes | RETIRE | The compat catalog's `adapter_owned_annotations` row is the brief's first action. No replacement annotation, filter or CORS configuration: the source policy stays in M1's structural model and `rest-cors` reproduces it on its own parity card. |
